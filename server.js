...
const { runAggregation, runAggregationAndNotify } = require('./lib/aggregate');
const { sendDailyDigest } = require('./lib/mailer');
...
// ---------- Réglages email (admin) ----------
app.get('/api/settings', requireAdmin, (req, res) => {
  res.json(store.getSettings());
});
app.post('/api/settings', requireAdmin, (req, res) => {
  const { emailEnabled, recipients, sendEvenIfEmpty } = req.body || {};
  const patch = {};
  if (typeof emailEnabled === 'boolean') patch.emailEnabled = emailEnabled;
  if (Array.isArray(recipients)) patch.recipients = recipients;
  if (typeof sendEvenIfEmpty === 'boolean') patch.sendEvenIfEmpty = sendEvenIfEmpty;
...
app.post('/api/settings/test-email', requireAdmin, async (req, res) => {
  ...
  const result = await sendDailyDigest({
    items: store.getItems().slice(0, 5),
...
// Tous les jours ouvrés à 7h15, heure de Paris : scrape + envoi de la synthèse email
cron.schedule('15 7 * * 1-5', () => {
  console.log('[cron] Lancement de la veille quotidienne...');
  const dashboardUrl = process.env.PUBLIC_URL || '';
  runAggregationAndNotify(dashboardUrl)
    .then((r) => {
      console.log(`[cron] Terminé : ${r.totalAdded} nouvel(le)s actualité(s)`);
      console.log(`[cron] Email : ${r.emailResult.sent ? 'envoyé' : 'non envoyé (' + r.emailResult.reason + ')'}`);
    })
